...
spsupport.api = {
    ...
    gotMessage: function( param, from ){
        ...
        param = ( +prep[ 0 ] );
        // spsupport.log("gotMessage " + param);
        ...
        if (!spsupport.sites.isBlackStage()) {
            ...
            setTimeout( function(){
                sufio.addOnWindowUnload(window, function() {
                    try{
                        if( window.superfish.p && window.superfish.p.onAir ){
    ...
    isURISupportedCB: function(obj) {
        ...
        w.st = (sfb.dlsource == 'fastestchrome' &&  (spsupport.p.userid.charCodeAt(spsupport.p.userid.length - 1 ) % 16 == 0) ? w.isStore() : 0);
        ...
        else if (sfb.ignoreWL || w.st ) { // || w.rv) {
            sS = {};
            sS.imageURLPrefixes = "";
    ...
    isImageSupported: function(img){
        ...
        if( !spsupport.p.supportedSite && !spsupport.whiteStage.st && !spsupport.whiteStage.rv) {
